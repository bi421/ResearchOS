...
def row(ts: int, **overrides):
    value = {"time": ts, "open": 100.0, "high": 101.0, "low": 99.0, "close": 100.5, "tick_volume": 10, "spread": 5, "real_volume": 0}
...
def test_valid_m1_rows_pass_without_forcing_24_7_continuity():
    ...
    report = validate_m1_rows([row(start), row(start + 60), row(start + 3600)], start_epoch=start, end_epoch=start + 3600)
...
def test_duplicate_timestamp_is_integrity_failure():
    start = 1_735_779_600
    report = validate_m1_rows([row(start), row(start)], start_epoch=start, end_epoch=start + 60)
...
def test_invalid_ohlc_is_rejected():
    start = 1_735_779_600
    report = validate_m1_rows([row(start, high=98.0)], start_epoch=start, end_epoch=start)
...
def test_invalid_volume_flags_count_each_bad_volume_field():
    start = 1_735_779_600
    report = validate_m1_rows([row(start, tick_volume=-1, real_volume=-2)], start_epoch=start, end_epoch=start)
...
def test_negative_volume_and_spread_are_rejected():
    start = 1_735_779_600
    report = validate_m1_rows([row(start, tick_volume=-1, spread=-2)], start_epoch=start, end_epoch=start)
...
def test_out_of_requested_range_is_rejected():
    start = 1_735_779_600
    report = validate_m1_rows([row(start - 60)], start_epoch=start, end_epoch=start + 60)
...
def test_nan_and_inf_are_rejected():
    start = 1_735_779_600
    report = validate_m1_rows([row(start, close=float("nan"), tick_volume=float("inf"))], start_epoch=start, end_epoch=start)
    assert report.invalid_ohlc_rows == 1
    assert report.invalid_volume_flags == 1
    assert not report.passed_integrity
...
def test_invalid_timestamp_is_rejected():
    start = 1_735_779_600
    report = validate_m1_rows([row("not-a-timestamp")], start_epoch=start, end_epoch=start)
...
def test_out_of_order_timestamps_are_rejected():
    start = 1_735_779_600
    report = validate_m1_rows([row(start + 60), row(start)], start_epoch=start, end_epoch=start + 60)
