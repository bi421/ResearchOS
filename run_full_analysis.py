...
def fetch_data(symbol, yf_symbol):
    try:
...
for name, yf_sym in SYMBOLS.items():
    ...
    df = fetch_data(name, yf_sym)
    if df is not None and not df.empty:
        asset_data[name] = df
        print(f'✅ {name}: {len(df)} records')

# Macro factor-уудын өгөгдөл
macro_data = {}
...
for macro_name, corr in macro_corr.items():
    if corr < -0.3:  # inverse correlation → Bullish for XAUUSD
        direction = ProbabilityOutcome.BULLISH
        strength = min(abs(corr), 1.0)
    elif corr > 0.3:  # positive correlation → Bearish for XAUUSD
        direction = ProbabilityOutcome.BEARISH
        strength = min(abs(corr), 1.0)
    else:
        direction = ProbabilityOutcome.NEUTRAL
...
for name, m in asset_metrics.items():
    # Find corresponding evidence item direction
...
- **Key Macro Driver:** {max(macro_corr.items(), key=lambda x: abs(x[1]))[0]} (corr: {max(macro_corr.items(), key=lambda x: abs(x[1]))[1]:.3f})
