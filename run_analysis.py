...
try:
    ...
    print("? CSV-??? ?????, ??? ???? ?????...")
    df = pd.concat([
        pd.read_csv(f, sep=';', header=None, names=['datetime','open','high','low','close','volume'], dtype={'datetime': str})
...
print("?? ???????")
print("="*70)
print(df_res[['timeframe','strategy','trades','winrate','winrate_ci_low','winrate_ci_high','total_return','sharpe']].to_string(index=False))
