...
df_h1 = (
    ...
    .agg([pl.col("Open").first().alias("open") if "Open" in df.columns else pl.col("open").first().alias("open"),
    ...
    .filter(pl.col("close").is_not_null()).sort("ts_utc")
)
print(f"H1 rows: {len(df_h1)}")

# SMA 50/200
df_h1 = df_h1.with_columns([pl.col("close").rolling_mean(50).alias("sma50"), pl.col("close").rolling_mean(200).alias("sma200")])
...
def metrics(d):
    if len(d)==0: return {}
    mean=d["strat_ret"].mean(); std=d["strat_ret"].std()
    sharpe=(mean/std*math.sqrt(252*24)) if std else 0
    cum=(1+d["strat_ret"].fill_null(0)).cum_prod(); peak=cum.cum_max(); dd=cum/peak-1
    return {"rows":len(d), "total_return_pct": round(float((cum[-1]-1)*100),2) if len(cum)>0 else 0, "sharpe": round(float(sharpe),3), "max_drawdown_pct": round(float(dd.min()*100),2)}
