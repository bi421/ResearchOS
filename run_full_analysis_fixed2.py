...
# ============================================================
# 1. ӨГӨГДӨЛ ТАТАХ (YFINANCE TICKER АРГААР)
# ============================================================
SYMBOLS = {
...
def safe_fetch(symbol, yf_symbol):
    try:
        ...
        if data.empty:
            ...
            return None
        # Close багана байгаа эсэх
        if 'Close' not in data.columns:
            print(f'⚠️  {symbol}: Close багана олдсонгүй')
            return None
...
for name, yf_sym in SYMBOLS.items():
    ...
    print(f'⏳ {name} ({yf_sym}) татаж байна...')
    df = safe_fetch(name, yf_sym)
    if df is not None and not df.empty:
        asset_data[name] = df
        print(f'✅ {name}: {len(df)} records')
    time.sleep(0.5)  # API хязгаарлалтаас зайлсхийх

macro_data = {}
for name, yf_sym in MACRO_SYMBOLS.items():
    print(f'⏳ Macro {name} ({yf_sym}) татаж байна...')
    df = safe_fetch(name, yf_sym)
    if df is not None and not df.empty:
        macro_data[name] = df
...
if not asset_data:
    print('❌ Ямар ч хөрөнгийн өгөгдөл татагдаагүй.')
    print('💡 Интернетийн холболт эсвэл тикерүүдийг шалгана уу.')
    print('💡 Жишээ нь: "GC=F" (Gold), "BTC-USD" (Bitcoin), "CL=F" (Oil)')
    sys.exit(1)
