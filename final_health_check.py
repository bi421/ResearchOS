...
# ============================================================
try:
    import researchos.quant_engine.backtest as be
    log("Модуль: quant_engine", "✅", "import амжилттай")
except Exception as e:
    log("Модуль: quant_engine", "❌", str(e)[:40])

try:
    import researchos.ml_engine.features as mf
...
try:
    ...
    files = glob.glob('data/raw/histdata/xauusd/DAT_ASCII_XAUUSD_M1_*.csv')
    if files:
        df = pd.concat([pd.read_csv(f, sep=';', header=None, nrows=1000,
                                    names=['datetime','open','high','low','close','volume']) 
                        for f in files[:2]], ignore_index=True)
...
try:
    ...
    if files:
        df = pd.concat([pd.read_csv(f, sep=';', header=None,
                                    names=['datetime','open','high','low','close','volume']) 
                        for f in files[:3]], ignore_index=True)
        df['datetime'] = pd.to_datetime(df['datetime'], format='%Y%m%d %H%M%S')
        df = df.set_index('datetime')
        ...
        if signals:
            res = vectorized_backtest_with_tpsl(
                prices=close.tolist(), signals=signals,
                initial_capital=100000, stop_loss_pct=0.02, take_profit_pct=0.04
            )
