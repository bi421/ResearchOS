"""
RIGOROUS macro-factor test.

Requires FRED_API_KEY in the environment when the cached US10Y file is absent.
...
if os.path.exists("us10y_real_2021_2025.csv"):
    ...
    if not FRED_API_KEY:
        raise RuntimeError("FRED_API_KEY is required when us10y_real_2021_2025.csv is absent")
...
print(f"Holdout period starts: {holdout_start_date.date()} ({len(df) - holdout_start_idx} rows held out)\n")
...
def run_horizon_test(data, factor_cols, horizon, lookback, only_after_idx=None):
    ...
    return {"n": n, "model_acc": model_acc, "baseline_acc": baseline_acc, "p_value": binom_result.pvalue}
...
for horizon in horizons:
    ...
    print(f"{horizon:>7}d {r['n']:>5} {r['model_acc']:>10.4f} {r['baseline_acc']:>10.4f} {r['p_value']:>10.4f} {str(sig):>10}")
