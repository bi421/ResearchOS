...
class LiveCandleValidator:
    ...
    def compare_candles(self, live: dict, historical: dict) -> dict:
        ...
        if abs(diff_pct) > 5:
            verification = {'status': 'WARNING', 'message': f'Close price differs by {diff_pct:.2f}%'}
        elif volume_ratio > 3:
            verification = {'status': 'NOTICE', 'message': f'Volume is {volume_ratio:.1f}x higher than historical'}
        return {
            'live': live,
