...
# =============================================
# 2. FEATURE GENERATION (шинэ + хуучин)
# =============================================
def add_new_features(df):
    ...
    df['target'] = close.shift(-1) / close - 1   # regression target
...
# =============================================
# 5. GRID SEARCH – threshold-ыг өгөөжийн түвшинд тохируулах
# =============================================
thresholds = [0.002, 0.003, 0.004, 0.005, 0.006]  # өгөөжийн босго
...
def eval_th(th):
    ...
    # Дохионоос probabilities-г хиймэл үүсгэх боломжгүй. Бид шууд signals-г бэктестэд дамжуулах хэрэгтэй.
    # run_ml_backtest_cpp нь probabilities-г хүлээдэг. Тиймээс бид энгийн векторжсон бэктест ашиглах эсвэл дохиог шууд боловсруулах функц бичих хэрэгтэй.
    # Энд бид дохиог шууд бэктест хийхгүй, харин энгийн Python бэктест ашиглах нь илүү хялбар.
    # Гэхдээ бид C++ хурдыг ашиглахыг хүсвэл дохио-г probability-д хувиргах хэрэгтэй.
    # Энгийн арга: Дээрх signals-ийг ашиглан vectorized_backtest-г дуудах.
    # from researchos.quant_engine.vectorized_backtest import vectorized_backtest
    from researchos.quant_engine.vectorized_backtest import vectorized_backtest
    res = vectorized_backtest(val_prices, signals)  # signals нь [('BUY', price), ...] хэлбэртэй
    return th, {'trades': res['num_trades'], 'sharpe': res['sharpe'], 'return': res['total_return'], 'dd': res['max_drawdown'], 'wr': res['win_rate']}
...
best_sharpe, best_th = -999, 0.004
for th, res in results:
    if res['trades'] >= 15 and res['sharpe'] > best_sharpe:
        best_sharpe, best_th = res['sharpe'], th
    print(f"{th:6.4f}  | {res['trades']:6d} | {res['return']:7.2%} | {res['sharpe']:6.2f} | {res['dd']:6.2%} | {res['wr']:7.2%}")
