...
# ============================================================
# 2. DATA ALIGNMENT (Ижил ажлын өдрүүд)
# ============================================================
all_data = {}
...
# Heatmap үүсгэх (base64)
plt.figure(figsize=(12, 10))
sns.heatmap(corr_matrix, annot=True, cmap='coolwarm', center=0, fmt='.2f')
plt.title('Cross-Asset Correlation Matrix (90 days)')
plt.tight_layout()
buf = io.BytesIO()
...
# ============================================================
report = f"""
# 📊 Multi-Asset Market Intelligence Report (v2)
**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M')}
**Period:** {start.strftime('%Y-%m-%d')} → {end.strftime('%Y-%m-%d')}
**Data Points:** {len(combined_df)} aligned trading days

---

## 🌍 Market Regime: **{regime}**
**Reasons:** {', '.join(regime_reasons) if regime_reasons else 'No clear signals'}
- **Risk-Off:** VIX > 20, DXY rising, BTC falling simultaneously
- **Risk-On:** Calm markets, VIX < 20, risk assets performing
...
**Key Insights:**
- **DXY vs XAUUSD:** {macro_corr.get('DXY', 0):.3f} ({"strong" if abs(macro_corr.get('DXY', 0)) > 0.7 else "moderate" if abs(macro_corr.get('DXY', 0)) > 0.4 else "weak"} inverse)
- **US10Y vs XAUUSD:** {macro_corr.get('US10Y', 0):.3f} ({"positive" if macro_corr.get('US10Y', 0) > 0.3 else "neutral"} correlation)
- **BTCUSD vs USOIL:** {corr_matrix.get('BTCUSD', pd.Series()).get('USOIL', 0):.3f} (risk-on/off correlation)
...
## 📌 Methodology Notes
- All data aligned to common trading days (Mon-Fri) to ensure accurate correlations.
- Annualization uses 252 trading days for equities, 260 for FX/metals, 365 for crypto.
- Regime detection based on VIX (>20), DXY trend, and BTC performance.
"""

# Хадгалах
with open('market_report_v2.md', 'w', encoding='utf-8') as f:
    f.write(report)

print('\n✅ Advanced report saved: market_report_v2.md')
