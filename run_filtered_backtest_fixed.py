...
# -------------------------------
# 2. NOISE FILTER (ADX + Volatility Regime + Kalman)
# -------------------------------
def add_filters(df):
    df = df.copy()
    ...
    # Volatility regime (z-score of rolling std)
    returns = close.pct_change()
    vol = returns.rolling(50).std()
    vol_z = (vol - vol.mean()) / vol.std()
    df['vol_filter'] = vol_z < 1.5
...
print(f"Valid trade periods: {df_h['valid_trade'].sum()} / {len(df_h)} ({df_h['valid_trade'].mean():.2%})")

# -------------------------------
# 3. FEATURES (техник үзүүлэлтүүд + фильтрүүд)
# -------------------------------
def make_features(df):
    ...
    df['valid_trade'] = df['valid_trade'].astype(int)
    # Target
...
feature_cols = [c for c in train_feat.columns if c not in ['target','datetime']]
...
# -------------------------------
# 6. SIGNAL GENERATION with FILTER GATE
# -------------------------------
def get_signals(df, probs, threshold):
    signals = []
    for i, idx in enumerate(df.index):
        if i < len(probs):
            if df.loc[idx, 'valid_trade']:  # gate
                prob = probs[i]
