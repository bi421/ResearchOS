...
class MillionCandleCPPAnalyzer:
    ...
    def load_all_csv(self):
        ...
        if not self.data_path.exists():
            ...
            # Өөр замд хайх
            alt_path = Path('researchos/data/raw/histdata/xauusd/')
            if alt_path.exists():
                self.data_path = alt_path
                print(f"   ✅ Өөр хавтас олдлоо: {self.data_path}")
            else:
                # Бүх боломжит замд хайх
                for root in ['.', '..', '../ResearchOS']:
                    p = Path(root) / 'data/raw/histdata/xauusd/'
                    if p.exists():
                        self.data_path = p
                        print(f"   ✅ Хавтас олдлоо: {self.data_path}")
                        break
                else:
        ...
        if not files:
            ...
            if all_files:
                print(f"   📄 Хавтасны агуулга: {[f.name for f in all_files[:5]]}")
            return False
