...
class EventWindowSpec:
    ...
    pre_windows: list[timedelta] = field(default_factory=lambda: [
        timedelta(hours=1),
        timedelta(minutes=30),
    ])
    post_windows: list[timedelta] = field(default_factory=lambda: [
        timedelta(minutes=30),
        timedelta(hours=1),
        timedelta(hours=4),
        timedelta(days=1),
    ])
    custom_windows: dict[str, tuple[timedelta, timedelta]] = field(
...
class EventTimeline:
    ...
    window_spec: EventWindowSpec = field(
        default_factory=EventWindowSpec
    )
    ...
    def _verify_no_overlaps(self) -> None:
        ...
        for i in range(len(self.events) - 1):
            ...
            if next_event.scheduled_time < current_end:
                raise ValueError(
                    f"Overlapping events: {current.event_id} and "
    ...
    def get_events_in_range(
        ...
        return [
            ...
            if start_utc <= event.scheduled_time <= end_utc
        ]
