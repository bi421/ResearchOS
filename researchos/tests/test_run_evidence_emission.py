...
def _make_run(
    ...
    run = ExperimentRun(
        ...
        simulation_config=sim or SimulationConfig(seed=42),
...
class TestRunPayload:
    ...
    def test_payload_excludes_timestamps_and_duration(self):
        payload = run_payload(_make_run(duration=12.5), experiment_hash="exp-hash-1")
        assert all(
            k not in payload
            for k in ("started_at", "completed_at", "duration_seconds", "created_at")
        )
...
class TestBuildRunEnvelope:
    ...
    def test_changed_dataset_config_different_artifact_hash(self):
        e1 = build_run_envelope(
            _make_run(dataset=DatasetConfig(source="yahoo", symbols=["AAPL"])),
            experiment_hash="exp-hash-1",
        )
        e2 = build_run_envelope(
            _make_run(dataset=DatasetConfig(source="google", symbols=["GOOG"])),
            experiment_hash="exp-hash-1",
        )
        assert e1.artifact_hash != e2.artifact_hash
    ...
    def test_runtime_timing_does_not_affect_hash(self):
        e1 = build_run_envelope(_make_run_with_timing(1), experiment_hash="exp-hash-1")
        e2 = build_run_envelope(_make_run_with_timing(99), experiment_hash="exp-hash-1")
        assert e1.artifact_hash == e2.artifact_hash

    def test_artifact_type_is_run(self):
        assert (
            build_run_envelope(_make_run(), experiment_hash="exp-hash-1").artifact_type
            == RUN_ARTIFACT_TYPE
            == "Run"
        )
    ...
    def test_scheme_version_is_2(self):
        ...
        assert build_run_envelope(_make_run(), experiment_hash="exp-hash-1").verify()
    ...
    def test_returns_immutable_envelope(self):
        assert isinstance(
            build_run_envelope(_make_run(), experiment_hash="exp-hash-1"), EvidenceEnvelope
        )
...
class TestExperimentLineage:
    def test_experiment_parent_preserved(self):
        ...
        assert "exp-hash-1" in envelope.parent_hashes
...
class TestEmitRun:
    ...
    def test_emit_rejects_non_run_type(self):
        ...
        with pytest.raises(ValueError):
            emit_run(build_envelope("Feature", {"x": 1}), repo)

    def test_emit_default_in_memory_repo(self):
        e = build_run_envelope(_make_run(), experiment_hash="exp-hash-1")
        assert emit_run(e).artifact_hash == e.artifact_hash


class TestExperimentRunLineage:
    def test_lineage_edge_experiment_to_run(self):
        repo = _make_repo()
        exp_hash = _seed_parent(repo)
        e = build_run_envelope(
            _make_run(),
            experiment_hash=exp_hash,
            parent_hashes=[exp_hash],
        )
        emit_run(e, repo)
        assert e.artifact_hash in repo.get_children(exp_hash)
        assert exp_hash in repo.get_parents(e.artifact_hash)
...
class TestProgressTracking:
    ...
    def test_acceptance_runtime_timing_no_effect(self):
        e1 = build_run_envelope(_make_run_with_timing(1), experiment_hash="exp-hash-1")
        e2 = build_run_envelope(_make_run_with_timing(500), experiment_hash="exp-hash-1")
        assert e1.artifact_hash == e2.artifact_hash
