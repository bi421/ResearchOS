"""
Tests for Phase 5.3b.5 — Validation Evidence Emission.

Covers:
    - ``build_validation_envelope`` determinism and identity.
    - Payload projection from the ``ValidationResult`` contract: validation
      hash, method/version, linked result_hash, linked run_hash, metrics,
      statistics (fold results), validation parameters, evaluation config.
    - Excludes timestamps / runtime telemetry / execution duration from
      identity.
    - Persistence + retrieval via ``EvidenceRepository``.
    - Result → Validation lineage edge (relation "validates").

Verification requirements:
    - identical validation → identical artifact_hash
    - changed validation metric → different artifact_hash
    - changed validation configuration → different artifact_hash
    - timestamps do NOT affect hash
    - Result → Validation lineage edge exists
    - repository retrieval works
    - tampered envelope rejected
    - payload remains primitive-only
...
class TestValidationPayload:
    def test_payload_preserves_content(self):
        ...
        payload = validation_payload(
            v, result_hash="result-hash-1", run_hash="run-hash-1"
        )
        assert payload["validation_hash"] == validation_hash(v)
    ...
    def test_payload_excludes_telemetry(self):
        v = _make_validation()
        payload = validation_payload(v, result_hash="result-hash-1")
    ...
    def test_payload_is_primitives_only(self):
        ...
        def _assert_primitives(value) -> None:
...
class TestBuildValidationEnvelope:
    ...
    def test_changed_metric_different_artifact_hash(self):
        v1 = _make_validation(metric=0.85)
        v2 = _make_validation(metric=0.90)
        e1 = build_validation_envelope(v1, result_hash="result-hash-1")
        e2 = build_validation_envelope(v2, result_hash="result-hash-1")
        assert e1.artifact_hash != e2.artifact_hash

    def test_changed_configuration_different_artifact_hash(self):
        v1 = _make_validation(fold_count=3)
        v2 = _make_validation(fold_count=5)
        e1 = build_validation_envelope(v1, result_hash="result-hash-1")
        e2 = build_validation_envelope(v2, result_hash="result-hash-1")
        assert e1.artifact_hash != e2.artifact_hash
    ...
    def test_changed_result_hash_different_artifact_hash(self):
        ...
        e2 = build_validation_envelope(
            _make_validation(), result_hash="result-hash-2"
        )
        assert e1.artifact_hash != e2.artifact_hash
    ...
    def test_artifact_type_is_validation(self):
        ...
        assert e.artifact_type == VALIDATION_ARTIFACT_TYPE == "Validation"
...
class TestResultLineage:
    def test_result_parent_preserved(self):
        ...
        assert "result-hash-1" in e.parent_hashes
...
class TestResultValidationLineage:
    def test_lineage_edge_result_to_validation(self):
        repo = _make_repo()
        result_hash = "result-hash-0001"
        e = build_validation_envelope(
            _make_validation(),
            result_hash=result_hash,
            parent_hashes=[result_hash],
        )
        emit_validation(e, repo)
        parents = repo.get_parents(e.artifact_hash)
        assert result_hash in parents
        children = repo.get_children(result_hash)
        assert e.artifact_hash in children
        assert repo.count_edges() == 1

    def test_emit_validation_for_result_links_lineage(self):
        repo = _make_repo()
        result_hash = "result-hash-0002"
        stored = emit_validation_for_result(
            _make_validation(), result_hash, repo
        )
...
class TestProgressTracking:
    """Track progress against the Phase 5.3b.5 acceptance criteria."""
    ...
    def test_acceptance_changed_metric_diff_hash(self):
        e1 = build_validation_envelope(
            _make_validation(metric=0.85), result_hash="result-hash-1"
        )
        e2 = build_validation_envelope(
            _make_validation(metric=0.90), result_hash="result-hash-1"
        )
        assert e1.artifact_hash != e2.artifact_hash

    def test_acceptance_changed_config_diff_hash(self):
        e1 = build_validation_envelope(
            _make_validation(fold_count=3), result_hash="result-hash-1"
        )
        e2 = build_validation_envelope(
            _make_validation(fold_count=5), result_hash="result-hash-1"
        )
        assert e1.artifact_hash != e2.artifact_hash
    ...
    def test_acceptance_result_to_validation_lineage(self):
        repo = _make_repo()
        result_hash = "result-hash-lineage"
        stored = emit_validation_for_result(
            _make_validation(), result_hash, repo
        )
        assert repo.get_children(result_hash) == [stored.artifact_hash]
