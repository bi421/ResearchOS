...
def _result(source: Path, tmp_path: Path, *, label: int = 0) -> Path:
    ...
    fold_model = {"sample_count": 2, "brier_score": 0.0, "log_loss": 0.0, "observed_rate": 0.5}
    fold_baseline = {"sample_count": 2, "brier_score": 0.25, "log_loss": 0.6931471806, "observed_rate": 0.5}
    result = {
...
def test_source_to_result_audit_accepts_valid_result(tmp_path: Path):
    output = audit(_source(tmp_path), _result(_source(tmp_path), tmp_path, label=0))
...
def test_source_to_result_audit_rejects_tampered_label(tmp_path: Path):
    ...
    output = audit(source, _result(source, tmp_path, label=1))
