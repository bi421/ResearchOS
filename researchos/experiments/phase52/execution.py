...
def _model_eval(estimator: Any, features, labels) -> tuple[ModelResult, list[int], list[dict[int, float]]]:
    ...
    accuracy = (
        sum(int(p) == int(a) for p, a in zip(predictions, labels)) / len(labels)
        if labels else 0.0
    )
...
def _run_prepared(prepared: Phase52PreparedData, cfg: Phase52Config) -> Phase52Result:
    ...
    if blocked is not None:
        return replace(
            ...
            metadata={
                **blocked.metadata,
                "prepared_sample_count": prepared.sample_count,
                "required_sample_count": cfg.train_size + cfg.validation_size,
            },
        )
    ...
    while start + cfg.train_size + cfg.validation_size <= len(features):
        ...
        validation_end = train_end + cfg.validation_size
        train_features = features[start:train_end]
        train_labels = labels[start:train_end]
        validation_features = features[train_end:validation_end]
        ...
        all_close.extend(prepared.close[i] for i in validation_source_indices)
        folds += 1
        if cfg.step_size <= 0:
            break
        start += cfg.step_size
    ...
    metadata = {
        ...
        "estimator": "EmpiricalProbabilityEstimator" if cfg.estimator_feature is not None else "MultivariateEmpiricalProbabilityEstimator",
