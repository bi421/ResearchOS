...
def _obs(day_index: int, close: float) -> DailyObservation:
    return DailyObservation(
        day=f"2020-01-{day_index + 1:02d}",
        timestamp=f"2020-01-{day_index + 1:02d}T00:00:00Z",
        open=close - 0.5,
        high=close + 1.0,
        low=close - 1.0,
        close=close,
...
def test_future_research_changes_only_future_labels_not_prior_features() -> None:
    context = tuple(_obs(i, 100.0 + i * 0.1) for i in range(60))
    research = tuple(_obs(60 + i, 106.0 + i * 0.2) for i in range(10))
...
def test_context_must_end_before_research() -> None:
    ...
    research = (context[-1],) + tuple(_obs(60 + i, 200.0 + i) for i in range(5))
