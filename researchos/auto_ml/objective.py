...
class Objective:
    ...
    def make_features(self, df, selected_cols, target_horizon=1):
        ...
        if selected_cols:
            keep_cols = [c for c in keep_cols if c in selected_cols] + [f'ret_lag_{l}' for l in [1,2,3,5,10,20] if f'ret_lag_{l}' in selected_cols]
        else:
            keep_cols = keep_cols + [f'ret_lag_{l}' for l in [1,2,3,5,10,20]]
    ...
    def __call__(self, trial):
        ...
        if use_macd: selected.extend(['macd', 'macd_signal', 'macd_diff'])
        if use_bb: selected.append('bb_pos')
        if use_vol: selected.append('volatility')
        if use_lags: selected.extend(['ret_lag_1','ret_lag_2','ret_lag_3','ret_lag_5','ret_lag_10','ret_lag_20'])
        ...
        test_feat = self.make_features(self.test_df, selected, target_horizon)
