...
def make_features(df, selected, target_horizon=1):
    ...
    if 'lags' in selected: keep_cols.extend(['ret_lag_1','ret_lag_2','ret_lag_3','ret_lag_5','ret_lag_10','ret_lag_20'])
...
class AutoMLObjective:
    ...
    def __call__(self, trial):
        ...
        if X_train.shape[0] < 100 or X_val.shape[0] < 50: return -999.0
        ...
        for i, idx in enumerate(self.val_df.index):
            if i < len(val_probs):
                ...
                if prob > threshold: signals.append(('BUY', price))
                elif prob < (1 - threshold): signals.append(('SELL', price))
        result = vectorized_backtest(self.val_df['close'].tolist(), signals)
        if result['num_trades'] < 20: return -999.0
