...
class OOSValidationResult:
    ...
    validation_method: str = "walk_forward_expanding"
...
def walk_forward_validate(
    ...
    *,
    ...
    """Evaluate a pre-specified condition with expanding walk-forward folds.

    No random shuffling is used. Test observations are never used to define the
    condition or estimate a threshold. Folds are strictly chronological.
    """
    ...
    while train_end + validation_size + test_size <= len(ordered):
        ...
        test = ordered[train_end + validation_size : train_end + validation_size + test_size]
    ...
    return OOSValidationResult(tuple(results), passed, len(results), stable, status, min_test_events)
