...
def wilson_proportion_ci(
    ...
    The implementation uses the standard normal quantile approximation with
    explicit validation. It is appropriate for reporting uncertainty around
    an empirical hit rate and does not imply independence beyond the sample.
    """
    if trials < 1:
    ...
    # Inverse standard-normal CDF using the relationship to erfinv is not
    # available in the Python standard library. This rational approximation is
    # deterministic and sufficiently accurate for reporting CIs.
    ...
    return ProportionEvidence(
        ...
        trials=trials,
        probability=p,
        confidence_interval=(max(0.0, centre - half_width), min(1.0, centre + half_width)),
