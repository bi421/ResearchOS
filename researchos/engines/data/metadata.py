"""
DatasetMetadata â€” comprehensive metadata for a historical dataset.
...
Guarantees:
    - Deterministic: Same dataset â†’ same metadata
...
class DatasetMetadata(BaseObject):
    ...
    def _to_hashable_dict(self) -> Dict[str, Any]:
        return {
            ...
            "quality": (
                self.quality.value if isinstance(self.quality, DataQuality) else self.quality
            ),
        ...
        base.update(
            {
                ...
                "status": (
                    self.status.value if isinstance(self.status, DatasetStatus) else self.status
                ),
                "dataset_hash": self.dataset_hash,
                "version": self.version,
