...
try:
    ...
    MT5_AVAILABLE = False
    print("âš ï¸ MetaTrader5 module not installed. Run: pip install MetaTrader5")
...
class MT5Connector:
        ...
        if MT5_AVAILABLE:
            try:
                if not mt5.initialize():
                    print("âš ï¸ MT5 initialization failed. Is terminal running?")
                else:
                    self.initialized = True
                    print("âœ… MT5 connected (primary data source).")
            except Exception as e:
                print(f"âš ï¸ MT5 init error: {e}")
    ...
    def fetch_ohlcv(
        ...
        if rates is None or len(rates) == 0:
            print(f"âš ï¸ No MT5 data for {symbol} {timeframe}")
            return pd.DataFrame()
    ...
    def compare(
        ...
        return {
            ...
            "data_quality": (
                ...
                else "GOOD" if corr > 0.95 and mape < 5.0 else "POOR"
            ),
        }
