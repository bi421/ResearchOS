"""
DatasetRepository â€” storage and retrieval interfaces for market data.
...
Guarantees:
    - Deterministic: Same data â†’ same storage results
...
from researchos.engines.data.contracts import DatasetStatus
from researchos.engines.data.dataset import HistoricalDataset
from researchos.engines.data.metadata import DatasetMetadata
...
class SqliteDatasetRepository(RepositoryInterface[T]):
    ...
    def _ensure_schema(self) -> None:
        ...
        try:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_datasets_symbol_timeframe "
                "ON datasets(symbol, timeframe)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_metadata_symbol_timeframe "
                "ON dataset_metadata(symbol, timeframe)"
            )
            cursor.execute(
    ...
    def _ensure_record_table(self, dataset_id: str, data_type: str) -> None:
        ...
        try:
            ...
            if data_type == "candle":
                ...
                """)
                cursor.execute(
                    f"CREATE INDEX IF NOT EXISTS idx_{table_name}_timestamp ON "
                    f"{table_name}(timestamp)"
                )
            else:
                cursor.execute(f"""
                    CREATE TABLE IF NOT EXISTS {table_name} (
    ...
    def _hydrate_records(self, dataset: HistoricalDataset) -> List[Any]:
        ...
        Records are stored in `records_<dataset_id>` (dashes â†’ underscores).
