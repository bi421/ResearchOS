...
Write-Host 'XAUUSD      : 2020-01-01 through 2021-04-30, M1'
Write-Host 'DXY         : 2020-01-01 through 2021-04-30, D1'
Write-Host 'Purpose     : extended feature warm-up context + source continuity audit'
...
try {
    ...
    npx --yes dukascopy-node -i xauusd -from 2020-01-01 -to 2021-04-30 -t m1 -f csv
    if ($LASTEXITCODE -ne 0) { throw "XAUUSD download failed with exit code $LASTEXITCODE" }

    Write-Host '[2/2] Downloading DXY D1 context...'
    npx --yes dukascopy-node -i dollaridxusd -from 2020-01-01 -to 2021-04-30 -t d1 -f csv
...
Write-Host ''
Write-Host 'Next step: run the context continuity/coverage audit, then the initialization convergence audit.'
