...
causes the observed sensitivity and verifies labels/source-day identity.
"""
...
def _feature_diffs(names: tuple[str, ...], context_rows: tuple[tuple[float, ...], ...], cold_rows: tuple[tuple[float, ...], ...], days: tuple[str, ...]):
    records = []
    for day, c_row, cold_row in zip(days, context_rows, cold_rows):
        for name, c, b in zip(names, c_row, cold_row):
            diff = abs(c - b)
            if not math.isclose(c, b, rel_tol=0.0, abs_tol=1e-12):
                records.append({"day": day, "feature": name, "context_value": c, "cold_start_value": b, "absolute_difference": diff})
...
def main(argv: list[str] | None = None) -> int:
    ...
    for feature_set in FEATURE_SET_NAMES:
        ...
        aligned_rows = tuple(seeded_by_day[d] for d in cold.source_days)
        aligned_labels = tuple(seeded_labels[d] for d in cold.source_days)
        if seeded.source_days != tuple(d for d in seeded.source_days):
            raise AssertionError("source-day sequence is not stable")
        days_identical = tuple(seeded.source_days) == tuple(cold.source_days)
        labels_identical = aligned_labels == cold.labels
        all_days_identical = all_days_identical and days_identical
        all_labels_identical = all_labels_identical and labels_identical
        records = _feature_diffs(cold.feature_names, aligned_rows, cold.rows, cold.source_days)
        by_feature: dict[str, dict[str, float | int]] = {}
        for r in records:
            x = by_feature.setdefault(r["feature"], {"count": 0, "max_absolute_difference": 0.0, "sum_absolute_difference": 0.0})
            x["count"] += 1
            x["max_absolute_difference"] = max(float(x["max_absolute_difference"]), float(r["absolute_difference"]))
            x["sum_absolute_difference"] += float(r["absolute_difference"])
        for x in by_feature.values():
            x["mean_absolute_difference"] = x["sum_absolute_difference"] / int(x["count"])
            del x["sum_absolute_difference"]
        affected_days = sorted({r["day"] for r in records})
        top = records[:20]
        global_top.extend({"feature_set": feature_set, **r} for r in top[:10])
        feature_sets[feature_set] = {
            "comparison_rows": len(cold.source_days),
            "feature_count": cold.feature_count,
            "differing_feature_values": len(records),
            "total_feature_values": len(cold.rows) * cold.feature_count,
            ...
            "source_days_identical": days_identical,
            "labels_identical": labels_identical,
            "top_differences": top,
            "by_feature": dict(sorted(by_feature.items(), key=lambda kv: (-int(kv[1]["count"]), -float(kv[1]["max_absolute_difference"]), kv[0]))),
            "context_audit_invariant_ok": audit.invariant_ok,
        }

    global_top.sort(key=lambda r: (-float(r["absolute_difference"]), str(r["feature_set"]), str(r["day"]), str(r["feature"])))
    payload = {
        "status": "PASS" if all_labels_identical and all_days_identical else "FAIL",
        ...
        "labels_identical_across_context_and_cold_start": all_labels_identical,
        "source_days_identical_across_context_and_cold_start": all_days_identical,
        "feature_sets": feature_sets,
        "global_top_differences": global_top[:50],
        "interpretation": "Forensic decomposition only; magnitude must be scientifically reviewed and is not automatically accepted as immaterial.",
    }
    ...
    for name, result in feature_sets.items():
        print(f"{name:20s}: affected_days={result['affected_research_days']} | differing={result['differing_feature_values']}/{result['total_feature_values']} | labels={result['labels_identical']}")
        if result["top_differences"]:
            top = result["top_differences"][0]
            print(f"  TOP                : {top['day']} / {top['feature']} / abs_diff={top['absolute_difference']:.12g}")
    print(f"LABELS IDENTICAL     : {payload['labels_identical_across_context_and_cold_start']}")
    print(f"SOURCE DAYS IDENTICAL: {payload['source_days_identical_across_context_and_cold_start']}")
