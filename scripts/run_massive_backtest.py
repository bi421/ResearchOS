# scripts/run_massive_backtest.py
import os
import sys
...
print("=" * 100)
print("🚀 MASSIVE BACKTEST – 6 Strategies × 7 Timeframes")
...
print(f"   Loaded {len(df)} D1 candles")
...
def bb_signal(df, period=20, std_dev=2):
    ...
    signal = np.where(df["close"] < bb_low, 1, 0)  # Buy when below lower band
    signal = np.where(df["close"] > bb_high, 0, signal)  # Sell when above upper band
    return pd.Series(signal, index=df.index).fillna(0)
...
def atr_signal(df, period=14, multiplier=1.5):
    ...
    # Buy when close > SMA + multiplier * ATR
