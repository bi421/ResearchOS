...
# Replicate load_macro_data
macro = {}

dxy = pd.read_csv(DXY_CSV, skiprows=2)
print(f"DXY raw columns: {list(dxy.columns)}")
print(f"DXY raw head:\n{dxy.head()}")
...
print(f"VIX raw columns: {list(vix.columns)}")
...
print(f"US10Y raw columns: {list(us10y.columns)}")
...
# Replicate create_features step by step
df = gold.copy()
df = df.sort_values("date").reset_index(drop=True)
print(f"After gold copy: {len(df)} rows")
...
print(f"After price/volume features: {len(df)} rows")
...
for name, macro_df in macro.items():
    ...
    after = len(df)
    print(f"After merging {name}: {after} rows (was {before})")

print(f"After macro merge: {len(df)} rows")

# Check for NaNs in merged columns
print(f'NaN in dxy_close: {df["dxy_close"].isna().sum()}')
print(f'NaN in vix_close: {df["vix_close"].isna().sum()}')
print(f'NaN in us10y_close: {df["us10y_close"].isna().sum()}')
...
print("NaN counts in feature_cols:")
...
if len(df_clean) > 0:
    print(f'First valid date: {df_clean["date"].iloc[0]}')
    print(f'Last valid date: {df_clean["date"].iloc[-1]}')
