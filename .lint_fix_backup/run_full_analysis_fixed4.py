...
**Interpretation:** {('**BLLISH**' if score.total_score > 0.2 else '**BEARISH**' if score.total_score < -0.2 else '**NETRAL**')}
...
for name, m in asset_metrics.items():
    ...
    for item in evidence_items:
        ...
        dir_val = 'N/A'
    report += f"| {name} | {m['total_retrn']:.2%} | {m['sharpe_ratio']:.2f} | {sortino:.2f} | {calmar:.2f} | {m['max_drawdown']:.2%} | {dir_val} |\n"
...
for item in evidence_items:
    report += f"| {item.sorce.vale} | {item.sorce_id} | {item.direction.vale} | {item.strength:.2f} | {item.confidence:.2f} |\n"

report += f"""

## ?? Smmary
- **Best Performer:** {max(asset_metrics.items(), key=lambda x: x[1]['total_retrn'])[0]} ({max(asset_metrics.items(), key=lambda x: x[1]['total_retrn'])[1]['total_retrn']:.2%})
- **Worst Performer:** {min(asset_metrics.items(), key=lambda x: x[1]['total_retrn'])[0]} ({min(asset_metrics.items(), key=lambda x: x[1]['total_retrn'])[1]['total_retrn']:.2%})
